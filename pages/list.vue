<template>
  <div class="relative light min-h-screen bg-white">
    <!-- Header Section -->
    <section
      class="py-12 md:py-16 bg-gradient-to-br from-python-blue-600 via-python-blue-500 to-python-yellow-500 relative overflow-hidden">
      <Container>
        <div class="text-center relative z-10">
          ...
          <p class="text-sm md:text-base text-white/90 max-w-2xl mx-auto mb-6 md:mb-8">
            Browse all available Python library compilers. Each compiler provides a dedicated environment optimized for
            its specific library.
          </p>

          <!-- Search Input -->
          <div class="max-w-xl mx-auto">
            <div class="relative">
              <input v-model="searchQuery" type="text" placeholder="Search compilers..."
              ...
              <Icon icon="ph:magnifying-glass"
                class="absolute left-4 md:left-5 top-1/2 -translate-y-1/2 w-5 h-5 md:w-6 md:h-6 text-gray-400" />
    ...
    <section v-if="filteredDefaultLibraries.length > 0" class="py-12 md:py-16 bg-white">
      <Container>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          <NuxtLink v-for="compiler in filteredDefaultLibraries" :key="compiler.path" :to="compiler.path"
            ...
            <div class="flex items-center justify-between">
              <div class="flex-1">
                <h3
                  class="text-lg md:text-xl font-semibold text-gray-900 mb-1 md:mb-2 group-hover:text-python-blue-600 transition-colors">
                  {{ compiler.name }}
                </h3>
    ...
    <section v-if="filteredAdditionalLibraries.length > 0" class="py-12 md:py-16 bg-gray-50">
      <Container>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
          <NuxtLink v-for="compiler in filteredAdditionalLibraries" :key="compiler.path" :to="compiler.path"
            ...
            <div class="flex items-center justify-between">
              <div class="flex-1">
                <h3
                  class="text-lg md:text-xl font-semibold text-gray-900 mb-1 md:mb-2 group-hover:text-python-yellow-600 transition-colors">
                  {{ compiler.name }}
                </h3>
                <p class="text-xs md:text-sm text-gray-600 leading-relaxed">
                  {{ compiler.description }}
    ...
    <!-- No Results -->
    <section v-if="searchQuery && filteredDefaultLibraries.length === 0 && filteredAdditionalLibraries.length === 0"
...
const defaultLibraries = [
  ...
  {
    name: 'Sankey Chart',
    description: 'Create Sankey diagrams online for free. Visualize flows, energy distribution, budgets, and processes with beautiful flow diagrams using Matplotlib\'s Sankey module.',
    path: '/sankey-chart'
  },
  {
    name: 'NumPy',
